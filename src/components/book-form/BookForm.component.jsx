...
const BookForm = ({ bookId, book }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const id = await bookService.save({ _id: bookId, ...formData });
...
const validators = {};
const defaultImageUrl = "https://bookstoreromanceday.org/wp-content/uploads/2020/08/book-cover-placeholder.png"; // Default placeholder image URL
...
const BookForm = ({ bookId, book }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Add default image only on submission if the field is empty
      const finalFormData = {
        ...formData,
      ...
      const isEditing = Boolean(bookId);
      const id = await bookService.save({ _id: bookId, ...finalFormData });
      const newBook = await bookService.getById(id);
